...
interface ViewCategoryListProps {
  editMode: boolean;
...
const ViewCategoryList: React.FC<ViewCategoryListProps> = ({
  editMode,
  selectedCategory,
  entriesData,
  entries,
  ...
  // useEffect to filter entries based on the selectedCategory
  useEffect(() => {
    const fetchData = async () => {
      try {
        const data = await entriesData;
        setEntries(data.filter((entry) => entry.category === selectedCategory));
      } catch (error) {
        console.error("Error fetching entries:", error);
      }
    };
    fetchData();
  }, [entriesData, selectedCategory]);
